...
The final output should be a single markdown file in the `flutter-triage/output/` directory, organized by the "Next steps" categories defined in flutter-pr-triage. For PRs, the script will automatically extract the accurate triage status, comprehensive summary, and actionable next steps from the individual PR markdown reports generated by `flutter-pr-triage`.
